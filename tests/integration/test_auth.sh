...
LOGIN_RESP=$(curl -s -X POST "$BASE/auth.php?action=login" \
...
WRONG=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/auth.php?action=login" \
    -H 'Content-Type: application/json' \
...
UNKNOWN=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/auth.php?action=login" \
    -H 'Content-Type: application/json' \
...
COOKIE_JAR=$(mktemp)
curl -s -c "$COOKIE_JAR" -X POST "$BASE/auth.php?action=login" \
    -H 'Content-Type: application/json' \
    -d '{"username":"testuser","password":"test1234"}' > /dev/null

REFRESH_RESP=$(curl -s -b "$COOKIE_JAR" -X POST "$BASE/auth.php?action=refresh")
...
NOCOOKIE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/auth.php?action=refresh")
[ "$NOCOOKIE" = "401" ] || fail "refresh without cookie: expected 401, got $NOCOOKIE"
pass

# ── 6. Logout clears the refresh cookie ────────────────────────────────────

LOGOUT_RESP=$(curl -s -b "$COOKIE_JAR" -c /dev/null -X POST "$BASE/auth.php?action=logout")
LOGOUT_OK=$(echo "$LOGOUT_RESP" | jq -r '.ok // false')
[ "$LOGOUT_OK" = "true" ] || fail "logout: expected ok=true"

# After logout, refresh should fail
POST_LOGOUT=$(curl -s -o /dev/null -w "%{http_code}" -b "$COOKIE_JAR" -X POST "$BASE/auth.php?action=refresh")
