...
export function bindEvents(handlers: MetaEventHandlers): void {
  ...
  // Remove custom field buttons (delegated)
  if (_customRows) {
    _customRows.addEventListener('click', (e: MouseEvent) => {
      ...
      if (removeBtn) {
        ...
        if (row) {
          ...
          _renderCustomRows(_pendingMeta.custom);
    ...
    // Custom field input changes
    _customRows.addEventListener('input', () => _onFieldChange(handlers.onDirty));
  }
}
...
function _renderForm(pm: PendingMeta): void {
  ...
  _renderCustomRows(pm.custom);
}

function _renderCustomRows(custom: Record<string, string>): void {
